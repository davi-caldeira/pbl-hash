...
public final class ChainingHashTable implements HashTable {
    ...
    public boolean insert(Registro r, Stats stats) {
        ...
        // colisões: percorrer até o fim (ou duplicata)
        Node cur = head;
        int traversed = 0;
        while (true) {
            if (cur.codigo == r.getCodigo()) {
                stats.collisionsInsert += traversed; // consideramos percurso até achar duplicata
                return false; // não insere duplicata
            }
        ...
        stats.collisionsInsert += traversed + 1; // +1 para a tentativa na aresta final
        n++;
        return true;
    }
